...
def bind_services_to_device(hass: core.HomeAssistant):
    ...
    async def cooker_stop(call):
        ...
        if not cookers:
            _LOGGER.warning(
                "Call service failed: Device(s) not found for %s", device_ids
            )
            return
...
async def async_setup_entry(
    ...
    if model not in SUPPORTED_MODELS:
        _LOGGER.error(f"Unsupported device found: {model}")
        return False
    ...
    if "coordinator" in hass.data[DOMAIN][config_entry.entry_id]:
        tokit_cooker_coordinator = hass.data[DOMAIN][config_entry.entry_id][
            "coordinator"
        ]
