...
from miio.integrations.chunmi.cooker_tokit.cooker_tokit import (
    CookerProfile,
...
async def async_setup_entry(
    ...
    coordinator: TokitCookerCoordinator = hass.data[DOMAIN][config_entry.entry_id][
        "coordinator"
    ]
    ...
    async def menu_changed(menu):
        ...
        duration_entity: TimeEntity = hass.data[DOMAIN][config_entry.entry_id][
            "entities"
        ][entity_id]
        await duration_entity.async_set_value(
            time(hour=duration // 60, minute=duration % 60)
        )
    ...
    menu_options_select_entity = MenuOptionsSelect(
        coordinator, device_info, device_name
    )
    hass.data[DOMAIN][config_entry.entry_id]["entities"][
        menu_select_entity.entity_id
    ] = menu_select_entity
    hass.data[DOMAIN][config_entry.entry_id]["entities"][
        menu_options_select_entity.entity_id
    ] = menu_options_select_entity
